...
int ARMCI_Create_mutexes(int count) {
  if (armci_mutex_grp != NULL)
    ARMCII_Error(__FILE__, __LINE__, __func__, "attempted to create ARMCI mutexes multiple times", 100);
...
int ARMCI_Destroy_mutexes(void) {
  ...
  if (armci_mutex_grp == NULL)
    ARMCII_Error(__FILE__, __LINE__, __func__, "attempted to free unallocated ARMCI mutexes", 100);
...
void ARMCI_Lock(int mutex, int proc) {
  if (armci_mutex_grp == NULL)
    ARMCII_Error(__FILE__, __LINE__, __func__, "attempted to lock on unallocated ARMCI mutexes", 100);
...
void ARMCI_Unlock(int mutex, int proc) {
  if (armci_mutex_grp == NULL)
    ARMCII_Error(__FILE__, __LINE__, __func__, "attempted to unlock on unallocated ARMCI mutexes", 100);
