...
int ARMCII_Iov_op_datatype(enum ARMCII_Op_e op, void **src, void **dst, int count, int elem_count,
    ...
    for (int start = 0; start < count; start += chunk) {
      ...
      switch(op) {
        case ARMCII_OP_PUT:
          gmr_put_typed(mreg, base_loc_ptr, 1, type_loc, MPI_BOTTOM, 1, type_rem, proc, NULL /* handle */);
          flush_local = 1;
          break;
        case ARMCII_OP_GET:
          gmr_get_typed(mreg, MPI_BOTTOM, 1, type_rem, base_loc_ptr, 1, type_loc, proc, NULL /* handle */);
          flush_local = 0;
          break;
        case ARMCII_OP_ACC:
          gmr_accumulate_typed(mreg, base_loc_ptr, 1, type_loc, MPI_BOTTOM, 1, type_rem, proc, NULL /* handle */);
          flush_local = 1;
          break;
        default:
          ARMCII_Error("unknown operation (%d)", op);
